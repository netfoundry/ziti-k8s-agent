...
func lookupEnvVars() {
	...
	if len(cert) == 0 {
		klog.Error(&MissingEnvVarError{variable: "TLS_CERT"})
		klog.Error(&MissingCmdLineVarError{variable: "TLS_CERT"})
	}
	...
	if len(key) == 0 {
		klog.Error(&MissingEnvVarError{variable: "TLS_PRIVATE_KEY"})
		klog.Error(&MissingCmdLineVarError{variable: "TLS_PRIVATE_KEY"})
	}
	...
	if zitiAdminCert == nil {
		klog.Error(&MissingEnvVarError{variable: "ZITI_ADMIN_CERT"})
		klog.Error(&MissingCmdLineVarError{variable: "ZITI_ADMIN_CERT"})
	}
	...
	if zitiAdminKey == nil {
		klog.Error(&MissingEnvVarError{variable: "ZITI_ADMIN_KEY"})
		klog.Error(&MissingCmdLineVarError{variable: "ZITI_ADMIN_KEY"})
	}
	...
	if len(searchDomains) == 0 {
		klog.Error(&MissingEnvVarError{variable: "SEARCH_DOMAINS"})
		klog.Error(&MissingCmdLineVarError{variable: "SEARCH_DOMAINS"})
